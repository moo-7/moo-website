...
async function getGithubDownloadCount () {
  ...
  if (shouldRegenRequest(cache)) {
    const resp = await (await fetch('https://api.github.com/repos/lrmn7/moo/releases')).json()
...
async function getGithubContributorsCount () {
  ...
  if (shouldRegenRequest(cache)) {
    ...
    const resp = await (await fetch('https://api.github.com/orgs/lrmn7/repos')).json()

    let contributors = {}

    for (const repo of resp) {
      const resp = await (await fetch(`https://api.github.com/repos/lrmn7/${repo.name}/contributors`)).json()
...
async function getDiscordCount () {
  const resp = await (await fetch('https://discord.com/api/guilds/1088032923443277824/widget.json')).json()
